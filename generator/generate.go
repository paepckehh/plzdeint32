...
const (
	...
	// components
	_header_nativ    = "// package plzdeint32\n// Code generated : German Postcode - DO NOT EDIT MANUALLY: "
	_package_nativ   = "package plzdeint32\n\n"
	_map_start_nativ = "// PLZ ... \nvar PLZ = map[int32]bool{\n"
	_map_end         = "}\n"
)
		...
		for i := 0; i < worker; i++ {
			go func() {
				for line := range feed_chan {
					...
					plz, err := strconv.ParseInt(s[0], 10, 32)
					if err != nil {
					...
					co_nativ_chan <- []byte("\t" + I32toS(int32(plz)) + ": true,\n")
				}
				bg.Done()
			}()
		}
